...
typedef enum    { DL_DATA, DL_ACK, DL_NEG }   FRAMEKIND;
...
void transmit_frame(MSG *msg, FRAMEKIND kind, size_t length, int seqno)
{
    ...
    case DL_NEG:
    {
      printf("NEG-ACK transmitted, seq=%d\n", seqno);
      break;
    }
...
EVENT_HANDLER(physical_ready)
{
    ...
    if(CNET_ccitt((unsigned char *)&f, (int)len) != checksum) {
      if(f.kind == DL_DATA)
      {
        printf("\t\t\t\tBAD checksum - request resend\n");
        transmit_frame(NULL, DL_NEG, 0, f.seq);
      }
      else
      {
       printf("BAD checksum");
  ...
  case DL_NEG:
  {
    printf("\t\t\t\tNEG-ACK received, seq=%d, ", f.seq);
